...
namespace soulcloud
{
    ...
    msgpack_reader::msgpack_reader(const uint8_t *data, size_t len)
    {
    ...
    mpack_type_t msgpack_reader::peek_type()
    {
    ...
    uint32_t msgpack_reader::expect_map()
    {
    ...
    uint32_t msgpack_reader::expect_array()
    {
    ...
    uint64_t msgpack_reader::expect_u64()
    {
    ...
    int64_t msgpack_reader::expect_int()
    {
    ...
    int32_t msgpack_reader::expect_int32()
    {
        const int64_t v = mpack_expect_int(&r_);
    ...
    bool msgpack_reader::expect_bool()
    {
    ...
    void msgpack_reader::expect_nil()
    {
        mpack_expect_nil(&r_);
        if (mpack_reader_error(&r_) != mpack_ok) fail(ERR_BAD_MSG);
    }

    double msgpack_reader::expect_float()
    {
        mpack_tag_t tag = mpack_peek_tag(&r_);
        if (mpack_reader_error(&r_) != mpack_ok) {
            fail(ERR_BAD_MSG);
    ...
    int32_t msgpack_reader::read_key(char *buf, size_t cap, size_t *out_len)
    {
    ...
    int32_t msgpack_reader::read_str(const char **ptr, uint32_t *len, uint32_t max_len)
    {
        mpack_tag_t tag = mpack_read_tag(&r_);
        if (mpack_reader_error(&r_) != mpack_ok) {
            fail(ERR_BAD_MSG);
    ...
    int32_t msgpack_reader::read_bin(const uint8_t **ptr, uint32_t *len, uint32_t max_len)
    {
        mpack_tag_t tag = mpack_read_tag(&r_);
        if (mpack_reader_error(&r_) != mpack_ok) {
            fail(ERR_BAD_MSG);
    ...
    msgpack_writer::msgpack_writer(uint8_t *buf, size_t cap)
    {
    ...
    void msgpack_writer::start_map(uint32_t count)
    {
    ...
    void msgpack_writer::finish_map()
    {
        if (err_ != ERR_OK) return;
        mpack_finish_map(&w_);
        if (mpack_writer_error(&w_) != mpack_ok) fail(ERR_OVERFLOW);
    ...
    void msgpack_writer::start_array(uint32_t count)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::finish_array()
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_str(const char *s)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_str(const char *s, size_t len)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_bin(const void *data, size_t len)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_uint(uint64_t v)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_int(int64_t v)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_bool(bool v)
    {
        if (err_ != ERR_OK) return;
    ...
    void msgpack_writer::write_double(double v)
    {
        if (err_ != ERR_OK) return;
    ...
    size_t msgpack_writer::bytes_written()
    {
        return mpack_writer_buffer_used(&w_);
    }

    int32_t msgpack_writer::finish()
    {
    ...
    namespace
    {
        ...
        int32_t read_arg_value(msgpack_reader &r, cmd_arg_value *out)
        {
            switch (r.peek_type()) {
            case mpack_type_nil:
                r.expect_nil();
        ...
        void write_arg_value(msgpack_writer &w, const cmd_arg_value *v)
        {
            switch (v->type) {
        ...
        void write_args(msgpack_writer &w, const cmd_arg *args, uint32_t count)
        {
