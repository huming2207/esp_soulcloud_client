...
namespace soulcloud
{
    const char *soulcloud_client::reset_reason_str(esp_reset_reason_t r)
    {
        switch (r) {
    ...
    int soulcloud_client::hex_val(char c)
    {
    ...
    void soulcloud_client::hex_to_bin(const char *hex, uint8_t *bin, size_t len)
    {
    ...
    bool soulcloud_client::topic_matches(const char *topic, size_t topic_len, const char *expected)
    {
    ...
    class soulcloud_client::client_impl
    {
    public:
    ...
    esp_err_t soulcloud_client::init(const config *cfg)
    {
        if (inited_) {
    ...
    esp_err_t soulcloud_client::start()
    {
    ...
    esp_err_t soulcloud_client::stop()
    {
        if (!inited_ || impl_ == nullptr) {
            return ESP_ERR_INVALID_STATE;
        }
    ...
    esp_err_t soulcloud_client::deinit()
    {
        if (!inited_ || impl_ == nullptr) {
            return ESP_ERR_INVALID_STATE;
        }
    ...
    esp_err_t soulcloud_client::register_command(const char *name, command_handler_t handler)
    {
        return command_registry::instance().register_command(name, handler);
    }
    ...
    void soulcloud_client::build_stat(uint8_t *buf, size_t cap, size_t *out_len)
    {
        char fw_hex[65] = {};
    ...
    esp_err_t soulcloud_client::report_stat()
    {
        if (!inited_ || impl_ == nullptr) {
            return ESP_ERR_INVALID_STATE;
        }
    ...
    void soulcloud_client::on_mqtt_connected()
    {
        connected_ = true;
    ...
    void soulcloud_client::on_mqtt_disconnected()
    {
        connected_ = false;
        if (conn_cb_ != nullptr) {
    ...
    void soulcloud_client::on_mqtt_data(const char *topic, size_t topic_len,
                                        const uint8_t *data, size_t data_len)
    {
        char expected[160] = {};
    ...
    void soulcloud_client::notify_wifi_connected()
    {
        if (impl_ != nullptr) {
