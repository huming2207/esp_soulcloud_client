...
namespace soulcloud
{
    ...
    void config_store::derive_serial(char *out, size_t cap) const
    {
        uint8_t mac[6] = {};
    ...
    esp_err_t config_store::load(config *out)
    {
        nvs_handle_t h = 0;
    ...
    esp_err_t config_store::set_string(const char *key, const char *value)
    {
        nvs_handle_t h = 0;
    ...
    esp_err_t config_store::set_u32(const char *key, uint32_t value)
    {
        nvs_handle_t h = 0;
        esp_err_t err = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &h);
        if (err != ESP_OK) {
