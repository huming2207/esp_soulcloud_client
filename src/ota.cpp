...
namespace soulcloud
{
    ...
    esp_err_t ota_executor::init(const config *cfg, mqtt_bridge *bridge)
    {
        if (cfg_ != nullptr) {
    ...
    void ota_executor::deinit()
    {
        cfg_ = nullptr;
        bridge_ = nullptr;
    }

    bool ota_executor::last_ota_matches(const char *release_id) const
    {
        nvs_handle_t h = 0;
    ...
    void ota_executor::store_last_ota(const char *release_id)
    {
        nvs_handle_t h = 0;
    ...
    esp_err_t ota_executor::start(const ota_notice *notice)
    {
    ...
    void ota_executor::task_trampoline(void *ctx)
    {
        static_cast<ota_executor *>(ctx)->run();
    }

    void ota_executor::report_state(const char *state, int32_t code, const char *message)
    {
        if (bridge_ == nullptr) {
            return;
        }
     ...
     */
    bool ota_executor::download_and_verify(esp_ota_handle_t *ota_handle_out,
